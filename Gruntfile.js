module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		exec: {
			...
			osx64: {
				command: '../idanodebuilds/build-osx.sh osx64'
			},
			osx32: {
				command: '../idanodebuilds/build-osx.sh osx32'
			}
		},
		watch: {
			options: {
		...
		concat: {
			...
			js: {
				...
				dest: 'public/idanode.js'
			},
			css: {
				src: ['src/css/*.css'],
				dest: 'public/css/idanode.css'
			},
			foundation: {
				src: [
		...
		uglify: {
			...
			prod: {
				files: {
					'public/idanode.js': ['public/idanode.js'],
				...
				files: [
					{
						expand: true,
						cwd: './webkitbuilds/',
						src: ['idanode.desktop', '../public/img/icons/icon-white-outline.ico', '../public/img/icons/icon-white-256.png'],
						dest: '../idanodebuilds/IdaNode/linux32/',
						flatten: true,
						filter: 'isFile',
						options: {timestamp: true, mode: true}
					},
					{
						...
						cwd: './webkitbuilds/',
						src: ['idanode.desktop', '../public/img/icons/icon-white-outline.ico', '../public/img/icons/icon-white-256.png'],
						dest: '../idanodebuilds/IdaNode/linux64/',
						flatten: true,
						filter: 'isFile',
						options: {timestamp: true, mode: true}
					},
		...
		nwjs: {
			options: {
				...
				macPlist: {
					CFBundleIdentifier: 'org.idanode.wallet',
					CFBundleURLTypes: [{
						CFBundleURLName: 'IdaNode action',
						CFBundleURLSchemes: ['idanode']
		...
		compress: {
			linux32: {
				options: {
					archive: '../idanodebuilds/idanode-linux32.zip'
				},
				expand: true,
				cwd: '../idanodebuilds/IdaNode/linux32/',
				src: ['**/*'],
				dest: 'idanode-linux32/'
			},
			linux64: {
				options: {
					archive: '../idanodebuilds/idanode-linux64.zip'
				},
				expand: true,
				cwd: '../idanodebuilds/IdaNode/linux64/',
				src: ['**/*'],
				dest: 'idanode-linux64/'
			}
		},
		browserify: {
			dist: {
		...
		debian_package: {
			linux64: {
				files: [
					{
						expand: true,
						cwd: '../idanodebuilds/idanode-test/linux64/',
						src: ['**/*'],
						dest: '/opt/idanode-test/'
					},
					//{expand: true, cwd: '../idanodebuilds/idanode-test/linux64', src: ['idanode.desktop'], dest: '/usr/share/applications/idanode-test.desktop'}
				],
				options: {
					maintainer: {
						name: 'IdaNode',
						email: 'idanode@idanode.org'
					},
