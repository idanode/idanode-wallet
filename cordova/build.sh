...
PROJECT="$BUILDDIR/../../idanodebuilds/project-$1"

if [ ! -d "$BUILDDIR/../../idanodebuilds" ]; then
  mkdir -p $BUILDDIR/../../idanodebuilds
fi
...
if [ ! -d $PROJECT ]; then
	...
	cordova create ../../idanodebuilds/project-$1 org.idanode.smartwallet IdaNode
	checkOK
...
#  checkOK
	...
	if [ $CURRENT_OS == "IOS" ]; then
		cordova plugin add https://github.com/idanode/phonegap-plugin-barcodescanner.git
		checkOK
...
#		cordova plugin add https://github.com/joyious/phonegap-plugin-barcodescanner.git
	...
	cordova plugin add https://github.com/idanode/CordovaClipboard.git
	checkOK
...
#	checkOK
	...
	cordova plugin add https://github.com/idanode/cordova-sqlite-plugin.git
	checkOK
...
if $DBGJS
then
	echo "${OpenColor}${Green}* Generating idanode bundle (debug js)...${CloseColor}"
	cd $BUILDDIR/..
	grunt cordova
	checkOK
else
	echo "${OpenColor}${Green}* Generating idanode bundle...${CloseColor}"
	cd $BUILDDIR/..
	grunt cordova-prod
	checkOK
fi
...
cp node_modules/idanode-common/initial.idanode.sqlite $PROJECT/www
cp node_modules/idanode-common/initial.idanode-light.sqlite $PROJECT/www
checkOK
...
if [ $CURRENT_OS == "ANDROID" ]; then
	...
	cat $BUILDDIR/android/android.css >> $PROJECT/www/css/idanode.css
...
#  checkOK
#
#  cp ios/Idanode-Info.plist $PROJECT/platforms/ios/Idanode-Info.plist
#  checkOK
#
#  mkdir -p $PROJECT/platforms/ios/Idanode/Resources/icons
#  checkOK
#
#  mkdir -p $PROJECT/platforms/ios/Idanode/Resources/splash
#  checkOK
#
#  cp -R ios/icons/* $PROJECT/platforms/ios/Idanode/Resources/icons
#  checkOK
#
#  cp -R ios/splash/* $PROJECT/platforms/ios/Idanode/Resources/splash
#  checkOK
fi
