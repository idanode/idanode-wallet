var BLACKBYTES_ASSET = require('idanode-common/constants').BLACKBYTES_ASSET;
var balances = require('idanode-common/balances');
...
function initWallet() {
	...
	function loadCompleteClient(showClient) {
		self._bIdanodeCoreLoaded = false; //"fix" : Looks like you are loading multiple copies of idanode core, which is not supported. Running 'npm dedupe' might help.
		...
		angularJs.onload = function() {
			var idanodeJS = document.createElement('script');
			idanodeJS.src = 'idanode.js';
			body.appendChild(idanodeJS);
			idanodeJS.onload = function() {
