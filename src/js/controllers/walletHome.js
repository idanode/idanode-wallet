...
var breadcrumbs = require('idanode-common/breadcrumbs.js');
...
angular.module('copayApp.controllers').controller('walletHomeController', function ($scope, $rootScope, $timeout, $filter, $modal, $log, notification, isCordova, profileService, lodash, configService, storageService, gettext, gettextCatalog, nodeWebkit, addressService, confirmDialog, animationService, addressbookService, correspondentListService, newVersion, autoUpdatingWitnessesList, go, safeApplyService) {
	...
	var chatStorage = require('idanode-common/chat_storage.js');
...
// 做旧版本兼容 升级后 内存中不存在值 或没有 2 写入2
	...
	var disablePaymentRequestListener = $rootScope.$on('paymentRequest', function (event, address, amount, asset, recipient_device_address) {
		...
		if (form.address.$invalid && !self.blockUx) {
			...
			self.resetForm();
			self.error = gettext('Could not recognize a valid Idanode QR Code');
		}
	});
	...
	$scope.openSharedAddressDefinitionModal = function (address) {
		...
		var ModalInstanceCtrl = function ($scope, $modalInstance) {
			...
			var walletDefinedByAddresses = require('idanode-common/wallet_defined_by_addresses.js');
			...
			profileService.requestTouchid(function (err) {
				...
				function composeAndSend(to_address, arrDefinition, assocSignersByPath) {
					...
					self.callApiToWalletHome = function (account, is_change, address_index, text_to_sign, cb) {
						...
						self.text_to_sign_qr = 'DND:' + JSON.stringify(obj);
						$timeout(function() {
					...
					fc.sendMultiPayment(opts, function (err) {
						...
						if (recipient_device_address) {  // show payment in chat window
							...
							if (binding && binding.reverseAmount) { // create a request for reverse payment
								...
								var paymentRequestCode = 'DND:' + my_address + '?amount=' + binding.reverseAmount + '&asset=' + encodeURIComponent(binding.reverseAsset);
...
//	form.amount.$render();
	...
	this.setFromUri = function (uri) {
		var objRequest;
		require('idanode-common/uri.js').parseUri(uri, {
			ifError: function (err) {
			},
	...
	this.openTxModal = function (btx) {
		...
		var ModalInstanceCtrl = function ($scope, $modalInstance) {
			...
			$scope.reSendPrivateMultiSigPayment = function () {
				var indivisible_asset = require('idanode-common/indivisible_asset');
				var wallet_defined_by_keys = require('idanode-common/wallet_defined_by_keys');
				var walletDefinedByAddresses = require('idanode-common/wallet_defined_by_addresses');
				var fc = profileService.focusedClient;
	...
	this.showCorrespondentListToReSendPrivPayloads = function (btx) {
		...
		var ModalInstanceCtrl = function ($scope, $modalInstance, $timeout, go, notification) {
			...
			$scope.sendPrivatePayments = function (correspondent) {
				var indivisible_asset = require('idanode-common/indivisible_asset');
				var wallet_general = require('idanode-common/wallet_general');
				indivisible_asset.restorePrivateChains(btx.asset, btx.unit, btx.addressTo, function (arrRecipientChains, arrCosignerChains) {
