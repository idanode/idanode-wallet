...
// 创建新钱包中---： 处理二维码 ： 观察钱包扫描冷钱包 /***************************************************/
	...
	this.handleQrcode = function parseUri(str, callbacks) {
		...
		if (obj_from_coldWallet.type) {
			switch (obj_from_coldWallet.type) {
				case "c1" :
					self.qrCodeColdwallet1 = "DND:" + JSON.stringify(obj_from_coldWallet);
					self.isErr = 0;
					self.isExists = 0;
	...
	self.askColdwalletQrcode = function () {
		...
		self.qrCodeColdwallet2 = "DND:" + JSON.stringify(self.obj_to_sign);  // 需要展示的信息（ 二维码 ）
