...
// 更改代码
		...
		if (profileService.focusedClient.isPrivKeyEncrypted()) {
			profileService.passWrongUnlockFC(null, function (err) {
				if (err == 'cancel'){  // 点击取消
					...
					self.signatureObj = "DND:" + JSON.stringify(self.signatureObj);
				}
			});
			return;
		}
	...
	eventBus.on('refresh_light_timeout', function () {
		...
		self.setOngoingProcess('Syncing', false);
		var lightWallet = require('idanode-common/light_wallet.js');
	...
	eventBus.on("signing_request", function (objAddress, top_address, objUnit, assocPrivatePayloads, from_address, signing_path) {
		...
		var bbWallet = require('idanode-common/wallet.js');
		var walletDefinedByKeys = require('idanode-common/wallet_defined_by_keys.js');
		...
		if(thirdAsset.length == 0) {
			...
			for(var i = 0; i < thirdAsset.length; i++) {
				if(self.arrAssetStore[thirdAsset[i]]) {
					...
					assocBalancesTemp[thirdAsset[i]]["issuserName"] = "DND user";
				...
				var req = https.request(options, function (res) {
					...
					res.on('data', function (data) {
						...
						if (data.indexOf('symbol') >= 0) {
							...
							assocBalancesTemp[thirdAsset[len]]["issuserName"] = data.entity.issuserName || "DND user";
							...
							assocBalancesTemp[thirdAsset[len]]["issuserName"] = "DND user";
	...
	this.csvHistory = function () {
		...
		$timeout(function () {
			fc.getTxHistory(self.arrBalances[self.assetIndex].asset, self.shared_address, function (txs) {
				...
				var data = txs;
				var filename = 'Idanode-' + (self.alias || self.walletName) + '.csv';
				var csvContent = '';
			...
			switch (obj_from_hotWallet.type) {
				case "h1" :   // ******************* （ 冷钱包 在认证页面 扫描的时候 ） 来自 观察钱包的二维码 （ 创建 观察钱包时候 ） ******************* //
					...
					if(obj_from_hotWallet.v == profileService.tmpeNum && obj_from_hotWallet.id == self.wallet_Id){
						self.myDeviceAddress = require('idanode-common/device.js').getMyDeviceAddress(); // 得到：设备地址
						self.ifQr = 1;
						self.objForHot = {
							"type": "c2",
						...
						self.objForHot = "DND:" + JSON.stringify(self.objForHot);
						self.scanErr = 0;
					}else{
						self.scanErr = 1;
						return;
